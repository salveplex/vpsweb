...
https.get('https://vosstaxi.no/Hjem/om/transportvilk-r', (res) => {
  let chunks = [];
  ...
  res.on('end', () => {
    ...
    const startIdx = lines.findIndex(l => l.includes('Transportvilkår for Voss Taxi SA'));
    const realText = startIdx !== -1 ? lines.slice(startIdx).join('\n') : text;

    // Convert &nbsp; back to space
    let finalText = realText.replace(/&nbsp;/g, '').replace(/\n\n+/g, '\n\n').trim();

    fs.writeFileSync('vilkar_clean.txt', finalText);
