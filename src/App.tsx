...
function Hero({ page, data, locale, isHome }: { page: CmsPage; data: SiteData; locale: Locale; isHome: boolean }) {
  ...
  return (
    <section
      className={`hero-stage relative overflow-hidden ${isHome ? 'min-h-[85dvh]' : 'min-h-[35dvh]'}`}
    ...
    >
      ...
      <div className={`hero-grid relative z-10 mx-auto ${isHome ? 'flex min-h-[85dvh] flex-col lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(390px,500px)] lg:items-center' : 'flex min-h-[35dvh] flex-col justify-end'} max-w-[92rem] gap-8 px-4 pb-12 pt-28 md:px-8 md:pb-16`}>
